...
class MomentumOptimizer(Optimizer):
    ...
    def get_weight_change(self, gradient: NDArray[float], layer: int, _) -> NDArray[float]:
        self.last_weights_changes[layer] = np.array(
            self.learning_rate * gradient + self.beta * self.last_weights_changes[layer])
        return self.last_weights_changes[layer]


class AdamOptimizer(Optimizer):

    def __init__(self, amount_of_layers: int, alpha: float = 0.001, beta_1: float = 0.9, beta_2: float = 0.999,
                 epsilon: float = 1e-8):
