...
class Layer:  # N neuronas, con M inputs
    ...
    def consolidate_weights(self):
        self.weights = np.sum(self.weights, self.pending_weight)
...
def backpropagation(layer_neurons: List[Layer], derivative_func: Activation_Function, expected_output: NDArray, input: NDArray, learning_bias: float) -> List[Layer]:
    ...
    for layer, i in layer_neurons[0:-1:-1]:
        ...
        gradient = np.multiply(derivative_func(layer.excitement), np.matmul(np.transpose(previous_layer.weights), previous_gradient))  
        curr_input = layer[i - 1].output if i != 0 else input
        # Nx1.1xM = NxM
